...
<head>
    ...
    <style>
        ...
        #answer { margin-top: 2rem; line-height: 1.5; }
        #answer h3 { margin-top: 1.5rem; }
...
async function submitQ() {
  ...
  if (!question) return;
  answerEl.innerHTML = '<em>Loading...</em>';
  ...
  let html = `<h3>Answer</h3>${marked.parse(data.answer)}`;
  if (data.references && data.references.length) {
    ...
    data.references.forEach(r => {
      ...
      const base = parts[0];
      let url;
      // If the source already looks like an absolute URL (http/https) just use it directly.
      if (/^https?:\/\//.test(base)) {
        url = base;
      } else {
        // encodeURI leaves path separators intact so nested folders work
        const filePath = encodeURI(base);
        url = `/doc/${filePath}`;
      }
      // Only append the page anchor for local PDF links that go through /doc/
      const page = parseInt(parts[1]);
      if (!isNaN(page) && url.startsWith('/doc/')) {
        url += `#page=${page+1}`;
      }
      html += `<p>[${r.index}] <a href="${url}" target="_blank">${r.source}</a>: ${r.snippet.slice(0, 120)}...</p>`;
    });
  }
...
async function submitMCP() {
  ...
  const question = qEl.value.trim();
  if (!question) return;
  answerEl.innerHTML = '<em>Loading MCP search...</em>';
  ...
  if (data.email_references && data.email_references.length) {
    html += '<h3>Email References</h3>';
    data.email_references.forEach(email => {
      const date = new Date(email.sent_at).toLocaleDateString();
      html += `<p><strong>${email.subject}</strong> (${email.from_address} – ${date})<br>${email.snippet.slice(0, 150)}...</p>`;
    });
  }
