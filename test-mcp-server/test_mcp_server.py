...
async def test_server():
    ...
    async with Client(MCP_SERVER_URL) as client:
        ...
        # Call pg_query tool
        print(">>> 🪛  Calling pg_query tool for counting number of documents")
        result = await client.call_tool("pg_query", {"sql": "SELECT COUNT(*) as document_count FROM documents;"})
        # print(result)
        print(f"<<< ✅ Result: {result.content[0].text}")
    
        # pg_query gives error if the query is not a read-only query
        print(">>> 🪛  Calling pg_query tool for updating nps of an email")
        result = await client.call_tool("pg_query", {"sql": "UPDATE emails SET nps = 10 WHERE id = '02sRi00000YOejtIAD';"})
        # print(result)
        print(f"<<< ✅ Result: {result.content[0].text}")
    
        # pg_explain
        print(">>> 🪛  Calling pg_explain tool for updating nps of an email")
        result = await client.call_tool("pg_explain", {"sql": "UPDATE emails SET nps = 10 WHERE id = '02sRi00000YOejtIAD';"})
        # print(result)
        print(f"<<< ✅ Result: {result.content[0].text}")
